...
data = [
        ('BIYRIM01', [[1,0,0], [0,1,0], [0,0,1]]), # Elastic, 90.124
        ('DAHLOQ', [[1,0,0], [0,1,0], [0,0,1]]), # Brittle
...
for d in data:
    ...
    # Directory
    folder = '3pf-6l-'+code+'-'+style+'_ind100'
    ...
    task = {'mode': 'bend',             # uni_xx_bulk, 3pf_xz_slab0
            'pbc_x': False,
            'type': 'single',           #
            'indenter_distance': 250.0, # A
            'border_distance': 10.0,    # A
            'inderter_t_hold': 300.0,   # ps, timesteps
            'indenter_rate': 1e-4,      # A/fs (10 m/s)
            'indenter_radius': 100.0,   # Ang
...
#    task['indenter_height'] = z_max + task['indenter_radius'] + delta
#    task['indenter_height_bottom'] = z_min - task['indenter_radius'] - delta
